...
class PathTrackerNode(Node):
    def __init__(self):
        ...
        # === 속도 제어 파라미터 ===
        self.max_speed = 0.5  # 직진은 최대 속력으로 할거임
        self.min_speed = 0.1  # 회전은 min~max 사이
        self.speed_lookahead = 45
        self.steer_threshold_low = 0.3 # 이 각도 이하면 최대 속도
    ...
    def listener_callback(self, msg):
        ...
        rear_x = current_x - 0.12 * math.cos(current_yaw) # 뒷바퀴 중심 x좌표.  차량 회전축~후방 길이 = 0.16인데 뒷바퀴 중심 몰라서 임의로 0.12라고 침
        rear_y = current_y - 0.12 * math.sin(current_yaw)
        ...
        # 2. 타겟 위치 갱신
        lookahead_step = 30 # 차량 뒷바퀴 + Lookahead만큼 앞부분 보기 (cm 단위)
    ...
    def calculate_goal_rad(self, fx, fy, ca, ga, gx, gy): # front_x, front_y, current_angle, goal_angle, goal_x, goal_y
        
        # 파라미터 정의

        pi = ga - ca # 현재 차선 곡률 - 차량 각도
        k = 1 # 횡방향 오차 gain값
